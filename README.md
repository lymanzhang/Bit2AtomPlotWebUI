...
Bit2AtomBot is a tool for interacting with the [AxiDraw
...
![a screenshot of the Bit2AtomBot user interface](docs/Bit2AtomBot.png)
...
2. **Install `Bit2AtomBot` globally:**

```bash
$ sudo npm install -g Bit2AtomBot
```
...
2. **Install `Bit2AtomBot`:**

```bash
$ npm install -g Bit2AtomBot
```
...
Start the `Bit2AtomBot` server from the terminal:
...
Go to `http://localhost:9080` if you're using the same computer where Bit2AtomBot is running.

Go to `http://<computer-ip>:9080` if you're on a different device.
(You can find the IP address by running `hostname -I` on the computer running Bit2AtomBot.)

---

### Running `Bit2AtomBot` over SSH

If you're connecting to your Raspberry Pi via SSH, it’s a good idea to keep the `Bit2AtomBot` server running inside a `tmux` session so it stays active even if your SSH session disconnects.
...
#### Start a `tmux` session and run `Bit2AtomBot`

```bash
$ tmux new -s Bit2AtomBot
$ Bit2AtomBot
```
...
```bash
$ tmux kill-session -t Bit2AtomBot
```

---

If you want `Bit2AtomBot` to run at boot on the Pi you can use a systemd unit file and enable the service:

```bash
sudo tee /lib/systemd/system/Bit2AtomBot.service <<EOF
[Unit]
Description=Bit2AtomBot
After=network.target

[Service]
ExecStart=Bit2AtomBot
Restart=always
User=pi
...
EOF
sudo systemctl enable Bit2AtomBot.service
```

To watch the logs while it is running, use:
```bash
journalctl -f -u Bit2AtomBot
```
...
If you want to connect to Bit2AtomBot from a web page that isn't served by Bit2AtomBot
...
a security precaution, but if you need it it's available. Just launch Bit2AtomBot with
the `--enable-cors` flag.

### Info

Bit2AtomBot makes use of the low-level `LM` command introduced in EBB firmware version
2.5.3 to carry out highly accurate constant-acceleration motion plans. If your
AxiDraw is running an older version of the firmware, Bit2AtomBot will fall back to the
less-accurate (but still pretty accurate) `XM` command.

To check what version of the EBB firmware your AxiDraw is running, run `Bit2AtomBot --firmware-version`:

```bash
$ Bit2AtomBot --firmware-version
...
To work on Bit2AtomBot, you can clone this repo and then run `npm start`:

```sh
$ git clone https://github.com/alexrudd2/Bit2AtomBot
$ cd Bit2AtomBot
$ npm run start
```
...
Bit2AtomBot's motion planning algorithm is heavily inspired by Michael Fogleman's
[axi](https://github.com/fogleman/axi) project.

Bit2AtomBot's UI would be an ugly mess if it weren't for [@kylestetz](https://github.com/kylestetz)'s discerning eye.
...
## Made with Bit2AtomBot

These images were plotted by folks using Bit2AtomBot. If you'd like to add something you've made here, [shoot me an email](mailto:nornagon@nornagon.net)!
