...
export type Hardware = "v3" | "brushless" | "nextdraw-2234" | "idraw-h-se" | "custom";
...
export class EBB {
  ...
  private write(str: string): Promise<void> {
    if (process.env.DEBUG_BIT2ATOM_COMMANDS) {
   ...
   * FIFO is set as deep as the board supports; BIT2ATOM_FIFO_DEPTH=n overrides,
   * and BIT2ATOM_FIFO_DEPTH=1 restores the boot default (the setting persists on
   * the board until power-cycled, so an explicit 1 is the only reliable "off").
   */
  public async configureFifoDepth(): Promise<void> {
    try {
      const requested = Math.floor(Number(process.env.BIT2ATOM_FIFO_DEPTH || 0));
