...
- **工作范围校验（防撞轴）**：设备档案新增机械可用行程（v3/brushless 430×300 mm，nextdraw-2234 559×864 mm，即 22×34 英寸）。服务端 `/plot` 扫描计划全部坐标（含落笔路径、抬笔空程与首尾行程，容差 0.5 步），超出行程即以 400 拒绝任务并明确提示超界方向与数值（如「X 方向最大坐标 804.0 mm 超出上限 430.0 mm」）。前端预览同步标红：纸张超出设备行程的区域显示红色遮罩，预览底部显示警示条。源于 2026-09-08 测试计划坐标超行程导致撞轴卡死的事故。回归测试覆盖四种场景：直接超行程计划、自定义纸张超行程（缩放流程）、不缩放模式 1:1 超行程、正常范围任务不受影响
- **自定义硬件安全工作区域参数**：硬件设置新增「工作区宽/高 (mm)」输入（保存在自定义硬件档案中）。custom 硬件的真实行程只有前端知道，随请求头（`X-Plot-Working-Area`）传给服务端参与超界校验——配置后同样硬拒绝，未配置时仅按 Axidraw 档案告警放行并在日志中提示补配置；前端预览对配置了工作区的 custom 硬件同样标红超界区域
...
- **缩放模式三态（排版设置）**：原「适配页面」复选框升级为三选一——「等比缩放到纸张」（默认，行为不变）、「按原尺寸绘制 (1:1)」（图形小于纸张绘图区域时不再被强制放大）、「自定义缩放比例」（按百分比缩放）。非 fit 模式可配合「裁剪至边距」移除超出纸张绘图区域的部分。控件从「更多设置」折叠面板移至常显的「排版设置」分区，与排版锚点逻辑一致；旧版持久化的 fitPage=false 自动迁移为「按原尺寸绘制」。CLI 对应新增 `--scale-percent`，`--fit-page` 关闭且比例非 100 时走自定义缩放
- **custom 硬件工作区配置的补全落盘**：`BaseDriver.plotWorkingAreaMm` 字段与硬件设置中「工作区宽/高 (mm)」输入的处理函数此前未随代码完整落盘（`setWorkingArea` 未定义，输入时会导致运行时错误），现补齐
- **导入时自动检测 SVG 真实物理尺寸**：导入时检查 SVG 根元素 `width` 是否带绝对物理单位（mm/cm/in/pt/pc/q），或 px 数值与 viewBox 不一致（如 2x/300dpi 导出），按 `width 实际毫米 ÷ viewBox 宽` 推算用户单位→毫米换算系数，传给规划器用于「按原尺寸/自定义缩放」模式——非 96dpi 导出的文件也能正确还原真实尺寸，不再必须按 96dpi 重新导出。`width="100%"`（Affinity 默认）或缺失时物理尺寸信息已丢失，仍按 96dpi 缺省处理。CLI `plot` 命令同样生效

### Fixed

- **导入 SVG 永久卡在「加载文件中...」**：导入真实尺寸检测改版时，`handleFile` 的 `reader.onload` 解构编辑未随代码落盘——`readSvg()` 新返回的对象被整体当作路径数组传给 `setPaths`，内部迭代直接抛 TypeError 中断 `onload`，加载状态永远无法复位。现修正调用，并给 `onload` 加 try/catch 兜底：解析异常会在控制台报错并复位加载状态，不再永久卡死
- **「裁剪至边距」后图层过滤索引错位，大量路径被误删**：`cropToMargins` 会把超出边距框的路径就地拆分成碎片，而其后的图层过滤仍按碎片数组下标回查原始路径（`inPaths[i]`）。单路径文件（Affinity 导出的单个超长 `<path>`，即整幅图一条折线）被拆成数百条碎片后下标越界，规划 worker 直接抛 TypeError 崩溃；多路径文件则索引错位，按别的路径的图层归属判定碎片去留，大量片段被误删——实测 `cloud13_smallSize` 自定义缩放 130% + 裁剪后仅剩 4.3 m（正确值应为 153.3 m）。现让每条路径携带原始索引贯穿裁剪与过滤，碎片继承原路径索引（隐藏线去除路径同样修正）。裁剪算法本身为逐线段与边距框求交（Liang-Barsky），区域内的节点与线段全部保留，仅切掉框外部分，无整体删除行为
- **自定义纸张尺寸后纸张下拉框回显为 USLetter**：自定义宽高后，下拉框未匹配任何预设时应显示「自定义」，但选项值（`自定义`）与回显值（`Custom`）不一致导致 React 回退显示第一项 USLetter，造成混淆。现统一选项值为 `Custom`，自定义纸张时正确回显「自定义」
