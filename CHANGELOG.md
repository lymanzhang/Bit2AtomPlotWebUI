...
- **工作范围校验（防撞轴）**：设备档案新增机械可用行程（v3/brushless 430×300 mm，nextdraw-2234 559×864 mm，即 22×34 英寸）。服务端 `/plot` 扫描计划全部坐标（含落笔路径、抬笔空程与首尾行程，容差 0.5 步），超出行程即以 400 拒绝任务并明确提示超界方向与数值（如「X 方向最大坐标 804.0 mm 超出上限 430.0 mm」）；custom 硬件档案非真实机型，仅告警不拒绝。前端预览同步标红：纸张超出设备行程的区域显示红色遮罩，预览底部显示警示条。源于 2026-09-08 测试计划坐标超行程导致撞轴卡死的事故
...
- **暂停回溯的任务日志缺少执行记录**：`ab89759` 中回溯执行行的编辑未随代码落盘，实测（cloud04）暂停回溯时日志仅有「收到恢复请求」而无执行记录。现补齐「回溯：进度 起点 → 目标（抬笔行程 xx mm）」一行（提交 `173c70d`）
- **任务日志预计绘制距离与实际距离口径不一致**：预计值按计划全部 XYMotion 累加（含抬笔空程），实际值只累计笔落段，两者可差数米（cloud04 实测预计 51.4 m / 实际 49.9 m，cloud13 曾反向偏差）。现预计值只统计笔落段（沿动作序列模拟笔状态，与实际统计逻辑一致），同一口径便于直接对比
